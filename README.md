...
│   ├── sfx.test.ts     音效测试
│   ├── useGame.ts      React 状态管理：撤销栈、尺寸切换、音效触发
...
覆盖四个层面：`engine.test.ts` 验证两种尺寸的游戏规则与不变量（含随机对局的模糊测试，检查坐标不越界、无重叠方块、分数自洽、数值均为 2 的幂；5×5 另有奇数宽度下的合并用例）；`storage.test.ts` 锁定存档按尺寸隔离、v1 旧存档迁移、以及尺寸不符或坐标越界时的拒绝行为；`sfx.test.ts` 用假的 `AudioContext` 验证音效确实创建了振荡器、音高单调递增、包络有淡入淡出、静音真的归零；`App.test.tsx` 在 jsdom 中渲染真实组件，验证键盘操作、触摸滑动、撤销、重开、静音、尺寸切换往返与持久化。
